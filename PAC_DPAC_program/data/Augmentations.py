...
class Augmentation:
    ...
    def __call__(self, x):
        x1 = self.weak_aug(x)
        x2 = self.strong_aug(x)
        x3 = self.ori_aug(x)
        return x1, x2, x3

def build_transform(is_train, name):
    if name == "cifar10":
    ...
    elif name == "imagenet10" or name == "imagenet_dogs":
        augmentation = Augmentation(
            img_size=224,
            val_img_size=256,
            s=0.5,
    ...
    return augmentation if is_train else augmentation.ori_aug
